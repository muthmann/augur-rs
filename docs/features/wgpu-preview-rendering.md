...
- Phase-4 source formats are:
  ...
  - time surface: `R8Unorm`
- A single fullscreen WGSL pipeline applies:
  - display min/max
  - gamma
  - LUT selection
  - mode-specific color mapping
- Time-surface decay remains CPU-prepared in this pass.
- Preview tools still read from `PreviewFrame`; this feature intentionally adds no GPU readback.
...
still full-frame synthesis, histogram work, and especially time-surface preparation.
...
- Full GPU event accumulation is still deferred.
