...
The ECF plugin is published in [prophesee-ai/hdf5_ecf](https://github.com/prophesee-ai/hdf5_ecf). No prebuilt releases are provided there, so CameraSDK includes a helper script at `scripts/install-ecf-plugin.sh` to clone, build, and install it locally.
